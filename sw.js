 ...
 * caches are cleared.
 */
const CACHE_VERSION = 'og-revise-v2';
const SHELL_CACHE   = CACHE_VERSION + '-shell';
const LIB_CACHE     = CACHE_VERSION + '-libs';

/* The app's own files. Relative paths resolve against the SW scope,
 * so this works whether the app lives at /OGinfo/ or elsewhere. */
const SHELL = [
  './',
  './index.html',
  './OG-Revise-PWA-v2.html',
        ...
        cache.match(req).then(hit => {
          ...
          return hit || network.then(res =>
            res || (req.mode === 'navigate'
              ? caches.match('./index.html').then(m => m || caches.match('./OG-Revise-PWA-v2.html'))
              : undefined)
          );
        })
      )
