 ...
 * -------------------------------------------------------------
 * The app is index.html at the folder root (/OGinfo/).
 * Goals:
 *   1. Installable + launchable offline (app shell cached).
 *   2. Serve the app + CDN libraries fast, from cache.
 *   3. NEVER touch Google / Drive / OAuth traffic — always network,
 *      never cached (auth-sensitive and must be fresh).
 *
 * Strategy:
 *   - App shell (same-origin files): stale-while-revalidate.
 *   - CDN libraries (fonts, pdf.js, docx-preview, jszip, mammoth):
 *       cache-first (versioned, immutable).
 *   - Everything Google: bypass the SW entirely.
 *   - Offline navigation fallback: the cached app (index.html).
 *
 * Bump CACHE_VERSION whenever you deploy a new app build.
 */
const CACHE_VERSION = 'og-revise-v3';
const SHELL_CACHE   = CACHE_VERSION + '-shell';
const LIB_CACHE     = CACHE_VERSION + '-libs';

/* The app's own files. Relative paths resolve against the SW scope,
 * so this works at /OGinfo/ or anywhere else. */
const SHELL = [
  './',
  './index.html',
